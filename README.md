[![GitHub tag (latest by date)](https://img.shields.io/github/v/tag/burakenez/gd32f4xx-cmake-vscode)](https://github.com/burakenez/gd32f4xx-cmake-vscode/tags/)

# GD32F4xx Demo Suites CMake Visual Studio Code Integration

🚀 Welcome to the **GD32F4xx Demo Suites CMake Visual Studio Code** repository! This project integrates the CMake build system with Visual Studio Code for **_[GD32F4xx Demo Suites V3.3.3](https://www.gd32mcu.com/en/download)_** and **_[GD32F4xx Firmware Library V3.3.3](https://www.gd32mcu.com/en/download)_**, running entirely inside a container — **no local toolchain installation needed**.
...
```bash
git clone https://github.com/burakenez/gd32f4xx-cmake-vscode.git
```
...
**Step 3 — Attach the GD-Link to WSL** *(each session)*
...
Find the GD-Link entry (typically `28e9:0189` or `28e9:058f`). Then bind *(first time only)* and attach:
```powershell
...
> ⚠️ **Attach the GD-Link to WSL _before_ opening or rebuilding the devcontainer.** The container bind-mounts `/dev/bus/usb` at startup; devices attached after the container starts will not be visible inside it.
...
### 2. **CMSIS/GD/GD32F4xx**
- **Version:** `V3.3.3 (2026-02-05)`
- **Path:** `Drivers/CMSIS/GD/GD32F4xx`
- **Source:** `GD32F4xx Demo Suites V3.3.3`
...
│           │   ├── Core
│           │   │   ├── Inc
│           │   │   │   ├── gd32f4xx_it.h      # Interrupt handler declarations
│           │   │   │   ├── gd32f4xx_libopt.h  # Library options
│           │   │   │   └── systick.h                                  # SysTick timer declarations
│           │   │   └── Src
│           │   │       ├── gd32f4xx_it.c      # Interrupt service routines
│           │   │       ├── main.c                                     # Application entry point
│           │   │       ├── system_gd32f4xx.c  # Clock and core initialisation
│           │   │       └── systick.c                                  # SysTick timer implementation
│           │   ├── Startup
│           │   │   └── startup_gd32f4xx.s     # Reset handler and vector table
...
│           ├── Drivers
│           │   ├── BSP/GD32F407R_START              # Board support package for GD32F407R_START
│           │   ├── CMSIS                                 # CMSIS for Cortex-M / GD32F4xx
│           │   └── GD32F4xx_standard_peripheral  # Standard peripheral library
│           ├── .clang-format                # Code style rules
│           ├── CMakeLists.txt               # Top-level CMake build file
│           ├── CMakePresets.json            # Debug / Release preset definitions
│           ├── gd32f4xx_flash.ld  # Linker script (memory regions)
│           └── GD32F4xx.svd             # SVD file for peripheral register views
...
> 🔧 xpack-openocd's bundled `libhidapi-hidraw` cannot access `/dev/hidraw*` inside Docker+WSL2 containers. The `Containerfile` replaces it with `libhidapi-libusb` (same API, accesses the GD-Link via `/dev/bus/usb` instead), which works correctly in this environment.
